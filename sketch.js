...
function drawNextGrid(nextGrid) {
    for (let j=0; j<rows; j++) {
        for (let i=0; i<columns; i++) {
            if (j+1 < rows && grid[i][j] > 0) {            
                if (grid[i][j+1] === 0) nextGrid[i][j+1] = grid[i][j];
                else {
                    if (i+1 < columns && i-1 >= 0) {
                        let direction = random([-1, 1]);
                        if (grid[i+direction][j+1] === 0) nextGrid[i+direction][j+1] = grid[i][j];
                        else if (grid[i-direction][j+1] === 0) nextGrid[i-direction][j+1] = grid[i][j];
                        else nextGrid[i][j] = grid[i][j];
                    }
                    if (i+1 < columns && grid[i+1][j+1] === 0) nextGrid[i+1][j+1] = grid[i][j];
                    else if (i-1 >= 0 && grid[i-1][j+1] === 0) nextGrid[i-1][j+1] = grid[i][j];
                    else nextGrid[i][j] = grid[i][j];
                }
            }
            else if (grid[i][j] > 0) nextGrid[i][j] = grid[i][j];
        }
    }
}
                ...
                for (let i=-bounds; i<bounds; i++) {
                    if (random(1) < 0.75) {
                        ...
                        if (col >= 0 && col < columns && row >= 0 && row < rows)
                            if (grid[col][row] === 0) grid[col][row] = hue;
                    }
                }
            }
        }
        else if (mouseI >= 0 && mouseI < columns && mouseJ >= 0 && mouseJ < rows) 
            if (grid[mouseI][mouseJ] === 0) grid[mouseI][mouseJ] = hue;
        hue += 0.5;
        if (hue > 360) hue = 1;
    }
}